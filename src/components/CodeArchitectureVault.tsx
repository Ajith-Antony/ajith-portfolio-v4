...
export default function CodeArchitectureVault() {
  ...
  return (
    <section id="vault" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 font-sans">
      ...
      <div className="text-center max-w-3xl mx-auto mb-12">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[var(--semantic-accent-bg)] border-2 border-[var(--semantic-accent)] text-[var(--semantic-accent)] text-xs font-mono mb-3 font-bold">
          ...
          <span>PRODUCTION REPOSITORY • REACT & TYPESCRIPT ARCHITECTURE</span>
        </div>
        <h2 className="text-3xl sm:text-5xl font-extrabold text-[var(--semantic-fg)] tracking-tight font-display">
          Code Architecture <span className="ds-gradient-text">Vault</span>
        </h2>
      ...
      <div className="ds-card p-6 sm:p-8 bg-[var(--semantic-surface-card)]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b-2 border-[var(--semantic-border-bold)] pb-6 mb-6">
          ...
          >
            {copied ? <Check className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4 text-[var(--semantic-fg-subtle)]" />}
        ...
        {/* Code View Box (Explicit Dark Workbench Container) */}

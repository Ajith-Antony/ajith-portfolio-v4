...
import { Activity, ArrowUpRight, ArrowDownRight, RefreshCw, Cpu, Zap } from 'lucide-react';
...
export default function InteractiveTradingTerminal() {
  ...
  useEffect(() => {
    ...
    const intervalMs = mode === 'websocket' ? 400 : 2500;
  ...
  return (
    <section id="trading-demo" className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 font-sans">
      ...
      <div className="text-center max-w-3xl mx-auto mb-12">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[var(--semantic-secondary-bg)] border-2 border-[var(--semantic-secondary)] text-[var(--semantic-secondary)] text-xs font-mono mb-3 font-bold">
          ...
          <span>COINROUTES EXPERIENCE • HIGH-FREQUENCY TRADING ENGINE</span>
        </div>
        <h2 className="text-3xl sm:text-5xl font-extrabold text-[var(--semantic-fg)] tracking-tight">
          Real-Time WebSocket <span className="ds-gradient-text">Orderbook Simulator</span>
        </h2>
        <p className="text-[var(--semantic-fg-muted)] text-sm sm:text-base mt-3 font-medium">
          Interactive demonstration of low-latency trading interfaces architected by Ajith Antony for institutional crypto trading.
        </p>
      </div>

      {/* Main Trading Widget Container */}
      <div className="editorial-panel-bold p-6 sm:p-8 relative overflow-hidden bg-[var(--semantic-surface-card)]">
        ...
        {/* Live Ticker Display Bar (High Contrast Dark Box) */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 ds-dark-workbench p-4 rounded-xl border-2 border-slate-700 mb-6 font-mono">
          <div>
            <span className="text-[10px] text-slate-300 block uppercase font-bold">Selected Instrument</span>
